...
const ZonesPage = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      commercialsData.forEach(c => commercialMap.set(c.id, {
        ...c,
      ...
      managersData.forEach(m => managerMap.set(m.id, {
        ...m,
        telephone: m.telephone || null
      }));

      const detailedZonesPromises = zonesData.map(async (zone) => {
        const details = await directeurSpaceService.getZone(zone.id);
  ...
  const handleZoneValidated = async (_data: {
    id?: string;
    ...
    try {
      // Note: La création/modification de zones pourrait nécessiter des endpoints spécifiques directeur
      console.warn('Création/modification de zone non implémentée pour le directeur');
    } catch (error) {
  ...
  const handleDelete = async () => {
    try {
      // Note: La suppression de zones pourrait nécessiter un endpoint spécifique directeur
      console.warn('Suppression de zones non implémentée pour le directeur');
      setItemsToDelete([]);
