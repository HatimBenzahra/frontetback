...
const ZoneDetailsPage = () => {
  ...
  const [zoneDetails, setZoneDetails] = useState<DirecteurZoneDetails | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (zoneId) {
      setLoading(true);
      directeurSpaceService.getZone(zoneId)
        .then(data => {
          setZoneDetails(data as DirecteurZoneDetails);
        })
        .catch(error => {
