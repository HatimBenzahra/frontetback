...
const TranscriptionsPage = () => {
  ...
  const loadAllCommercials = useCallback(async (forceRefresh = false) => {
    try {
      ...
      const url = `${BASE}/directeur-space/transcriptions/commercials${forceRefresh ? `?t=${Date.now()}` : ''}`;
  ...
  const checkAutoBackup = useCallback(async () => {
    try {
      const token = localStorage.getItem('access_token');
      const response = await fetch(`${BASE}/directeur-space/transcriptions/check-auto-backup`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          ...(token && { 'Authorization': `Bearer ${token}` })
  ...
  const backupToS3 = useCallback(async () => {
    try {
      ...
      const token = localStorage.getItem('access_token');
      const response = await fetch(`${BASE}/directeur-space/transcriptions/backup-to-s3`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          ...(token && { 'Authorization': `Bearer ${token}` })
  ...
  const handleSelectCommercial = (id: string) => {
    ...
    if (container) {
      ...
      setTimeout(() => {
        navigate(`/directeur/transcriptions/${id}`);
