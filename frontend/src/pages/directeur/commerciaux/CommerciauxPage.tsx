...
const CommerciauxPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const [commerciauxFromApi, , managersFromApi] = await Promise.all([
        ...
        setManagers((managersFromApi || []).map(m => ({ ...m, telephone: m.telephone ?? null })));
  ...
  const columns = useMemo(
    () => createColumns(isDeleteMode, handleEditOpen as any, handleViewDetails as any, handleOpenAssignModal as any) as any,
    [isDeleteMode, handleEditOpen, handleViewDetails, handleOpenAssignModal]
  );
  ...
  const handleConfirmAssign = async () => {
    ...
    try {
      await adminService.updateCommercial(assignTarget.id, {
  ...
  const handleUpdateCommercial = useCallback(async () => {
    ...
    try {
      ...
      await adminService.updateCommercial(id, {
        nom,
        prenom,
        email,
        managerId,
