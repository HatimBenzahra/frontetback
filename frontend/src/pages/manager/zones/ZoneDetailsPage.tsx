...
import { managerZoneService, type ManagerZoneDetailsFromApi } from '@/services/manager-zone.service';
...
const ZoneDetailsPage = () => {
  ...
  const [zoneDetails, setZoneDetails] = useState<ManagerZoneDetailsFromApi | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (zoneId) {
      setLoading(true);
      managerZoneService.getManagerZoneDetails(zoneId)
        .then(data => {
          setZoneDetails(data);
        })
        .catch(error => {
