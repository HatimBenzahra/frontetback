...
const ManagerTranscriptionsPage = () => {
  ...
  const loadManagerCommercials = useCallback(async () => {
    try {
        ...
        data.forEach((transcription: any) => {
          ...
          if (!commercialMap.has(commercialId)) {
            commercialMap.set(commercialId, {
              id: commercialId,
              name: commercialName || commercialId,
              sessionsCount: 0,
              lastTime: 0
            });
          }
      ...
      console.error('Erreur chargement commerciaux manager:', error);
    }
  }, [BASE]);

  useEffect(() => {
    loadManagerCommercials();
  }, [loadManagerCommercials]);
  ...
  const refreshAllData = useCallback(async () => {
    try {
      ...
      console.log('🔄 Rafraîchissement complet des données manager...');
      
      // Recharger les commerciaux du manager
      await loadManagerCommercials();
      ...
      console.log('✅ Rafraîchissement manager terminé');
    } catch (error) {
      console.error('❌ Erreur lors du rafraîchissement manager:', error);
    } finally {
      setIsRefreshing(false);
    }
  }, [loadManagerCommercials, socket]);
  ...
  const handleSelectCommercial = (id: string) => {
    ...
    if (container) {
      ...
      setTimeout(() => {
        navigate(`/manager/transcriptions/${id}`);
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              Gestion et consultation des sessions de transcription de vos commerciaux
          ...
          <CardHeader className="pb-4 bg-gradient-to-r from-blue-50 to-indigo-50 border-b border-gray-100/50">
            <div className="flex items-center justify-between">
              <CardTitle className="flex items-center gap-2 text-lg">
                ...
                Mes commerciaux ({commercials.length})
              </CardTitle>
              <div className="flex items-center gap-2">
...
export default ManagerTranscriptionsPage;