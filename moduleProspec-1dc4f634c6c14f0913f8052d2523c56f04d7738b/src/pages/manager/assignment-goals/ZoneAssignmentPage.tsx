...
import { ManagerZoneAssignmentCard } from '@/components/page-components/ManagerZoneAssignmentCard';
...
export default function ZoneAssignmentPage({ 
  ...
  useEffect(() => {
    const fetchAssignmentData = async () => {
      try {
        const [assignmentsData, historyData] = await Promise.all([
          managerAssignmentGoalsService.getManagerAssignments(),
          managerAssignmentGoalsService.getManagerAssignmentHistory(),
        ]);

        // Utiliser les vraies données du service
        setAssignmentsStatus(assignmentsData);
  ...
  const handleAssignZone = async (
    ...
    try {
      await managerAssignmentGoalsService.assignZoneToManager(
        zoneId, 
        assigneeId, 
        assigneeType, 
        startDate, 
      ...
      // Rafraîchir les deux sources de données
      const [refreshedAssignments, refreshedHistory] = await Promise.all([
        managerAssignmentGoalsService.getManagerAssignments(),
        managerAssignmentGoalsService.getManagerAssignmentHistory(),
      ]);
      
      setAssignmentsStatus(refreshedAssignments);
      setAssignmentHistory(refreshedHistory || []);
  ...
  const handleStopAssignment = async (assignmentId: string) => {
    try {
      await managerAssignmentGoalsService.stopManagerAssignment(assignmentId);
            ...
            <>
              ...
              {assignmentsStatus?.assignments && assignmentsStatus.assignments.length > 0 ? (
                <DataTable
                  ...
                  data={assignmentsStatus.assignments.map((assignment: any) => ({ ...assignment, id: assignment.id }))}
                  title=""
                  noCardWrapper={true}
                />
              ) : (
