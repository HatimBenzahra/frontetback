...
const EquipeDetailsPage = () => {
    ...
    if (equipeId) {
      ...
      const fetchAllData = async () => {
        ...
        setAllCommerciaux(commerciauxData.map(comm => ({
          ...
          generateZonePerformanceData(equipeData, assignedZonesData);
  ...
  const generateZonePerformanceData = (_equipeData: EquipeDetailsFromApi, zones: any[]) => {
    ...
    <div className="space-y-5 p-6 pb-4 bg-gradient-to-br from-white via-green-50/60 to-green-100/30">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            ...
            Équipe {equipeDetails.nom}
                ...
                <p className="text-green-200 mt-1 text-sm">
                  Manager : {equipeDetails.manager} • {equipeDetails.stats.nbCommerciaux} commerciaux
                </p>
              </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
              ...
              <div>
                ...
                <p className="font-medium">{equipeDetails.manager}</p>
              </div>
            </div>
            <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
              ...
              <div>
                ...
                <p className="font-medium">{equipeDetails.stats.nbCommerciaux}</p>
              </div>
            </div>
            <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
        <StatCard 
          ...
          value={equipeDetails.stats.contratsSignes} 
          Icon={Handshake} 
          color="text-emerald-500" 
        />
        <StatCard 
          title="RDV Pris" 
          value={equipeDetails.stats.rdvPris} 
          Icon={Briefcase} 
          color="text-sky-500" 
        />
        <StatCard 
          title="Refus" 
          value={(equipeDetails.stats as any).refus || 0} 
          Icon={X} 
          color="text-red-500" 
        />
        <StatCard 
          title="Immeubles Prospectés" 
          value={(equipeDetails.stats as any).immeublesProspectes || 0} 
          Icon={Building} 
          color="text-orange-500" 
        />
        <StatCard 
          title="Portes Prospectées" 
          value={(equipeDetails.stats as any).portesProspectees || 0} 
          Icon={DoorOpen} 
              ...
              <div key={commercial.id} className="p-4 border border-gray-200 rounded-lg hover:shadow-md transition-shadow bg-white">
                ...
                <div className="mt-3 pt-3 border-t border-gray-100">
                  <Button 
                    ...
                    size="sm" 
                    className="w-full"
                    onClick={() => navigate(`/admin/commerciaux/${commercial.id}`)}
                  >
                    <Users className="h-4 w-4 mr-2" />
                    Voir détails
                  </Button>
