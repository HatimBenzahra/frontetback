...
export default function AssignmentGoalsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [zonesData, commercialsData, managersData, equipesData, historyData, globalGoal] = await Promise.all([
  ...
  const handleAssignZone = async (
    ...
    try {
      ...
      // Refresh history - récupérer tout l'historique
      const refreshed = await assignmentGoalsService.getAssignmentHistory();
